...
=======
  <div class="h-screen md:flex md:flex-row mt-10">
    {{-- Left Side --}}
    <div class="h-full md:w-7/12 bg-black flex items-center">
      <img src="{{asset('storage/' . $post->image)}}" class="max-h-screen object-cover mx-auto" alt="{{$post->description}}"  >
    </div>
    {{-- Right Side --}}
    <div class="flex w-full flex-col bg-white md:w-5/12">
      {{-- Top --}}
        ...
        <div class="flex items-center p-5">
          ...
          @if ($post->owner->id === auth()->id())
            <form action="/p/{{$post->slug}}/delete" method="POST">
              ...
              @method('DELETE')
              <button type="submit" onclick="return confirm('Are you sure?')">
                <i class='bx bx-message-x ml-1 text-xl text-red-600'></i>
              </button>
            </form>
          @endif
        </div>
      ...
      {{-- Buttom --}}
      <border-t-2 class="p-5">
        <form action="/p/{{$post->slug}}/comment" method="POST">
          @csrf
          <div class="flex flex-row">
            <textarea name="body" id="comment_body" placeholder="Add a Comment..." class="h-10 grow resize-none overflow-hidden border-none bg-none p-0 placeholder-gray-400 outline-0 focus:ring-0" cols="30" rows="10"></textarea>
            <button class="ms-5 border-none bg-white text-blue-500">Post</button>
          </div>
        </form>
      </border-t-2>
